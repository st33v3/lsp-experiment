...
class BenchTraversal:
  ...
  def simpleString(b: Blackhole): Unit =
    for (j <- 0 until BenchTraversal.str.length()) do
      b.consume(BenchTraversal.str.charAt(j))
  ...
  def longSequencer(b: Blackhole): Unit = 
    val seq: Sequencer = new LongSequencer(BenchTraversal.str)
    while !seq.eof() do
  ...
  def intSequencer(b: Blackhole): Unit = 
    val seq: Sequencer = new IntSequencer(BenchTraversal.str)
    while !seq.eof() do
      b.consume(seq.current)
      seq.consume()